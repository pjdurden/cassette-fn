...
verifying the recording instead of your code. `llm-tape` records at the function
...
- `options.mode` (`'auto' | 'record' | 'replay' | 'off'`) - default `'auto'`.
  ...
  - If `process.env.LLM_TAPE_MODE` is set, it overrides `options.mode` entirely.
...
`llm-tape` wraps one function you choose, the one that actually makes a network call
